...
%   by SBP and JS.
%   Last updated Oct. 11, 2019, using MATLAB R2018b
%
%   Description:
...
%
%   BACKUP VERSION NOTE:
%       created before experimenting with expanded calendar merging options
%   
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DEV NOTES
% 2019/10/11
...
function runValidation(...
    ...
    dirPath_plots = fullfile(dirPath_out,['plots_',targetName]);
    ...
    nMat = numel(matNames);
    matStarts = datetime(NaN(nMat,6));
    for ii = 1:nMat
        matStarts(ii) = TWD_Common.Utilities.readDateTime(matNames{ii});
    end
    
    % Get ID and/or event spreadsheets.
    % If ID spreadsheet already exists, load it and resume.
    % If ID spreadsheet does not exist, create a new one from events.
    % Either way, will want to keep the events sheet too. 
    dtFormat = 'yyyyMMdd_HHmmss';
    xlsName_events = sprintf('%s_%s_RawEvents',depName,targetName);
    xlsName_IDs = sprintf('%s_%s_Validated',depName,targetName);
    xlsPath_events = fullfile(dirPath_out,[xlsName_events,'.xlsx']);
    xlsPath_IDs = fullfile(dirPath_out,[xlsName_IDs,'.xlsx']);
    if logical(exist(xlsPath_IDs,'file'))
        % load existing ID spreadsheet
        xlsOutData = readtable(xlsPath_IDs);
    ...
    elseif logical(exist(xlsPath_events,'file'))
        % create new ID spreadsheet
        xlsOutData = createIDSpreadsheet(xlsPath_events,dtFormat,eventMergeOpt,eventMergeVal,caller);
        iStart = 1;
    else
        % No files found
        warning('No Event or ID spreadsheets found')
        return
    end
    % get events sheet
    ...
    hfMain = createMainWindows(caller,clickDiscrimData.Obj);
            ...
            while ~doNextEvent
                switch state
                    case 'reset'
                        ...
                        % do summary plots
                        plotSummaryFigs(hfMain.Summary,data,depName,eStartTimeii);
                        if doOverlayPlot
                    ...
                    if nClicksTarget > 0
                        plotClick(hfMain.Click,hfMain.ClickSeq,data,clickStartTarget,clickSortOpt,click_iSort,clickImg,Fs,figNameBaseii,caller);
                    ...
                    clickPrompt = sprintf('CLICK PLOTS: enter =%s, d =Change Direction, o =Change Order, r =Reset, s =Save',clickStepStr);
                    switch caller
                        case 'BWD'
                            actPrompt = 'OTHER ACTIONS: f =Toggle Param Status, p =Change Discriminator, c =Add Comment, u =Update Spreadsheet, e =Change Event, q =Quit';
                        case 'SWD'
                            actPrompt = 'OTHER ACTIONS: f =Toggle Param Status, c =Add Comment, u =Update Spreadsheet, e =Change Event, q =Quit';
                    end
                    ...
                    switch opt
                        ...
                        case 'o'
                                % change sorting order (and reset)
                            ...
                            case 'u'
                                ...
                                saveTable(xlsOutData,xlsOutPath)
    ...
    % emergency saving for unexpected errors
        ...
        saveTable(xlsOutData,xlsOutEPath)   
...
%% createIDSpreadsheet ----------------------------------------------------
function xlsOutData = createIDSpreadsheet(xlsPath_events,dtFormat,eventMergeOpt,eventMergeVal,caller)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % read events sheet
    xlsInDataRaw = readtable(xlsPath_events);
    
    % merge events if necessary
    [xlsInDataNew,targetTime] = mergeEvents(xlsInDataRaw,dtFormat,eventMergeOpt,eventMergeVal,caller);
    
    % create output table
    nEvents = height(xlsInDataNew);
    ...
    xlsOutData = [xlsInDataNew(:,1:2),timeTable,xlsInDataNew(:,3:end),IDAppendTable];
end
...
%% createMainWindows ------------------------------------------------------
function hf = createMainWindows(caller,clickDiscrim)
...
%   PP also.
    ...
    for ii = 1:nMat
        ...
        if nSegmentsii > 0
            ...
            % to the true end.
            recEndii = matStartii + seconds(dataii.rawDur);
            ...
            if nClicksInRangeii > 0
                ...
                for jj = 1:nDiscrim
                    ...
                    [passedjj,clickParamPassjj,clickParamValsjj] = discrimjj.evalBeakedClicks(dataii);
...
%% plotSummaryFigs --------------------------------------------------------
function plotSummaryFigs(hf,data,depName,tStart)
% Adapted from "bw_combined_analysis_verify"
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    % isolate data of interest
    ICIBeaked = diff(data.pos(:,1))*1000;
    FPeakBeaked = data.peakFr;
    bw10dbBeaked = data.bw10db;
    bw3dbBeaked = data.bw3db;
    specClickBeaked = data.specClick;
    specNoiseBeaked = data.specNoise;
    nBeaked = numel(FPeakBeaked);
    Fs = data.fs;
    
    %SBP: delete ici not relevant to real ici
    ICIBeakedAdj = ICIBeaked(ICIBeaked>50 & ICIBeaked<1000);
    
    %SBP: find ici histogram bin with highest count (most likely to be true ici)
    ICIBins = 0:10:1000;
    [ICICounts,ICIEdges] = histcounts(ICIBeakedAdj,ICIBins);
    ...
    FPeakBins = 0:1:100;
    [FPeakCounts,FPeakEdges] = histcounts(FPeakBeaked,FPeakBins);
    ...
    peakMed = median(FPeakBeaked);
    med10db = median(bw10dbBeaked,1);
    med3db = median(bw3dbBeaked,1);
    
    % plot graphs
    %SBP: sort spectras by peak frequency and prepare for plotting concetanated spectrogram
    [~,iSortFPeak] = sort(FPeakBeaked);
    specSorted = specClickBeaked(iSortFPeak,:);
    ...
    meanSpecClickBeaked = TWD_Common.computeMeanSpectrum(specClickBeaked);
    meanSpecNoiseBeaked = TWD_Common.computeMeanSpectrum(specNoiseBeaked);
    ...
    ha1 = subplot(2,2,1);
    histogram(ha1,FPeakBeaked,FPeakBins,'EdgeColor','none','FaceColor','k','FaceAlpha',1)
    xlim(ha1,[0,125])
    ...
    % subplot 2: ICI
    ha2 = subplot(2,2,2);
    histogram(ha2,ICIBeakedAdj,ICIBins,'EdgeColor','none','FaceColor','k','FaceAlpha',1)
    xlim(ha2,[0,1000])
    ...
    ha3 = subplot(2,2,3);
    ha3.NextPlot = 'add';
    plot(ha3,f,meanSpecClickBeaked,'LineWidth',2)
    plot(ha3,f,meanSpecNoiseBeaked,':k','LineWidth',2) 
    % ylim([-100 -20])
    xlim(ha3,[5,95])
    a3YLim = ha3.YLim;
    ...
    title(ha3,sprintf('Mean click spectra, n=%d',nBeaked),'FontWeight','bold')
    text(ha3,0.5,0.9,sprintf('-10dB lower = %d kHz',round(med10db(1))),'Unit','Normalized');
    
    % subplot 4: concatenated spectrogram
    ha4 = subplot(2,2,4);
    imagesc(ha4,1:nBeaked, f, specSorted); axis xy; colormap(gray)
    ylim(ha4,[0,100]);
...
%% plotClick --------------------------------------------------------------
function plotClick(hfClick,hfClickSeq,data,clickStarts,sortOpt,iSort,img,Fs,figNameBase,caller)
...
% for e = 800 (SWD original), winStartEndTime(2) = 3.196;
    ...
    axis xy; colormap(blue); view(0,90); ylim([0 100]); 
    xlim(xRange);
...
%% showClickParams --------------------------------------------------------
function showClickParams(hf,valTableBeaked,paramPassTablesBeaked,discrimPassMatBeaked,discrimNames,isDiscrimActive,tClickStartBeaked,iSort,img,showParamPass,figNameBase)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    iClick = iSort(img);
    %%% features
    paramNames = valTableBeaked.Properties.VariableNames';
    paramVals = table2cell(valTableBeaked(iClick,:))';
    nParams = numel(paramVals);
    %%% discrimination
    nDiscrim = numel(discrimNames);
    discrimPass = discrimPassMatBeaked(iClick,:)';
    ...
    if showParamPass
        ...
        for ii = 1:nDiscrim
            paramPassTableii = paramPassTablesBeaked{ii};
    ...
    tClickStart = tClickStartBeaked(iClick);
...
%% saveTable --------------------------------------------------------------
function saveTable(xlsOutData,xlsOutPath)
