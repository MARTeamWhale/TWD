...
%   by SBP and JS.
%   Last updated Mar. 15, 2023, using MATLAB R2018b (update incomplete)
%
%   Description:
                    ...
                    case 'viewclicks'
                        ...
                        switch opt
                            ...
                            case 'jc'
                                ...
                                % need to activate callback functions for
                                % hovering over concatenated spectrogram
                                % plot. When finished, turn all the 
                                % callbacks back into empty strings.
                                
                                % Axes objects have a CurrentPoint property
                                % that returns the position of the mouse
                                % cursor. This could be accessed using a
                                % ButtonDownFcn for either the Image object
                                % or the Axes it is in.
                                
                                % I wonder if a larger concatenated
                                % spectrogram should be plotted in a new 
                                % window upon typing 'j', as with the
                                % discriminators? Perhaps it should be
                                % plotted according to how clicks are
                                % currently sorted? But what about click
                                % indices? They are dependent on the sort
                                % order... perhaps there should be an
                                % "absolute" click index and a "relative"
                                % index? [The indexing issue has been
                                % somewhat addressed]
                                
                                % If wanting to draw a bar highlighting
                                % where the cursor is currently hovering,
                                % then will need to make use of the
                                % WindowButtonMotionFcn callback from the
                                % parent FIGURE (axes do not have this
                                % callback)
                                
                                % perhaps I should highlight a point on the
                                % concatenated spectrogram regardless of
                                % whether the user hovers over it or not,
                                % to show which click is being looked at?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [fNames,~] = TWD_Common.Utilities.getFileNames(dirPath_results,'xlsx',false);
