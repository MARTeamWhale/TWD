...
%   Last updated Sep. 27, 2019, using MATLAB R2016b
%
%   Description:
...
function compileClicks(dirPath_analysis,dirPath_audio,depName,nfft,nClicksMax,segDur)
    ...
    fileNames_ctg = TWD_Common.Utilities.getFileNames(dirPath_metadata,'cTg');
    fileNames_wav = TWD_Common.Utilities.getFileNames(dirPath_audio,'wav');
    ...
    if isfolder(dirPath_out)
        ...
        matNames = TWD_Common.Utilities.getFileNames(dirPath_out,'mat');
        if ~isempty(matNames)
    ...
    for ii = iStart:nFiles
        ...
        % get file paths
        filePathii_wav = fullfile(dirPath_audio,fileNames_wav{ii});
        filePathii_ctg = fullfile(dirPath_metadata,fileNames_ctg{ii});
        ...
        if nClicksii > 0
            ...
            iClickPartEndsii = [nClicksMax:nClicksMax:nClicksii,nClicksii]';
